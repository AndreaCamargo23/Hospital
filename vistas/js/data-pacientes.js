$(document).ready(function() {
    ...
    tablaPaciente = $('#tablaPaciente').DataTable({
        ...
        "columns": [
            ...
            { "data": "rh" },
            { "data": "estado" },
            { "defaultContent": "<div class='text-center'><div class='btn-group'><button class='btn btn-success btn-sm btnEditar' title='Editar'><i class='fas fa-edit'></i></button><button class='btn btn-dark btn-sm btnBorrar' title='Gestionar Salida'><i class='fas fa-times'></i></button></div></div>" }
        ]
    });
    ...
    $(document).on("click", ".btnBorrar", function() { //Cuando se le de salida al paciente se efectuara la factura del paciente y quedara en estado de deuda hasta que la factura este pagada 
        ...
        id = parseInt(fila.find('td:eq(0)').text()); //trae el codigo en la posicion 0);
        opcion = 3;
        var confirmacion = confirm("¿Esta seguro que ya se le dio de alta al paciente con número de documento " + id + "?");
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data == "null") {
                        ...
                        Swal.fire({
                            icon: 'info',
                            title: 'El Paciente se encuentra en Deuda',
                        });
                        tablaPaciente.ajax.reload(null, false);
                    }
                }
